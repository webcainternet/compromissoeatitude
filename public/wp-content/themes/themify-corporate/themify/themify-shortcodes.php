...
function themify_shortcode( $atts, $content = null, $code = '' ) {
	switch ( $code ) {
		...
		case 'themify_col':
			...
			return "<div class='shortcode col$grid'>" . do_shortcode( preg_replace( array( '#<p>\s*+(<br\s*/*>)?\s*</p>#i', '~\s?<p>(\s|&nbsp;)+</p>\s?~' ), '', force_balance_tags( $content ) ) ) . '</div>';
		break;
		case 'sub_col':
...
function themify_shortcode_box( $atts, $content = null ) {
	...
	$boxstr = '<!-- shortcode box --> <div class="shortcode clearfix box ' . $style . '">' . do_shortcode( preg_replace( array( '#<p>\s*+(<br\s*/*>)?\s*</p>#i', '~\s?<p>(\s|&nbsp;)+</p>\s?~' ), '', force_balance_tags( $content ) ) ) . '</div> <!-- /shortcode box -->';
	return $boxstr;
}
