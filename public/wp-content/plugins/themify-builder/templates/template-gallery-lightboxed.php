...
if( '' != $thumbnail_gallery ) {
	...
	// get the first image in gallery, use that as the thumbnail
	$thumbnail = wp_get_attachment_image_src( $gallery_images[0]->ID, 'full' );
	$thumbnail = themify_get_image( "ignore=true&src={$thumbnail[0]}&w={$thumb_w_gallery}&h={$thumb_h_gallery}" );
}
