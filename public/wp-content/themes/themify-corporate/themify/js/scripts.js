...
jQuery(document).ready(function($){
	...
	$('.preview-icon').each(function(){
		var $self = $(this), $parent = $self.parent(), $val = $parent.find('.val'),
			$group = $self.closest('fieldset').find('.hide-if-none');
		$self.click(function(e){
			e.preventDefault();
			var wasNone = 'none' == $val.val(),
			...
			if ( 'none' == $val.val() ) {
				if ( $group.length <= 0 ) {
					$group = $self.closest('.themify_field_row').nextUntil('div:not(.hide-if-none)').filter('.hide-if-none');
				}
				if ( $group.length > 0 ) {
					// Fade out, then collapse.
					$group.one(transitionEvents, function(){
				...
				if ( $group.length > 0 ) {
					...
					$group.filter('.toggled-off').removeClass('conceal');
				}
			}
		});
		if ( 'none' == $val.val() ) {
			...
			if ( $group.length > 0 ) {
				$group.addClass('conceal collapse');
			}
		}
	});
