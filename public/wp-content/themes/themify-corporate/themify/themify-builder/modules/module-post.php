...
class TB_Post_Module extends Themify_Builder_Module {
	...
	public function get_options() {
		...
		$builder_cpt = get_option( 'builder_cpt', array() );
		$post_types = get_post_types( array( 'public' => true, 'publicly_queryable' => 'true' ), 'objects' );
		/* Exclude attachments, Builder layouts and layout parts, and also custom post types in Builder that have their own module from the list */
		$excludes = array_merge( $builder_cpt, array( 'attachment', 'tbuilder_layout', 'tbuilder_layout_part' ) );
		$post_types_options = array();
		foreach( $post_types as $key => $type ) {
			if( ! in_array( $key, $excludes ) ) {
				$post_types_options[$key] = $type->labels->singular_name;
			}
		}
		...
		$term_options = array();
		$excludes = array( 'post_format', 'highlight-category', 'portfolio-category', 'testimonial-category', 'slider-category' );
		foreach( $taxonomies as $key => $tax ) {
			if( ! in_array( $key, $excludes ) ) {
				$taxonomy_options[$key] = $tax->labels->name;
				$term_options[] = array(
					'id' => "{$key}_post",
		...
		$taxonomy_options['post_slug'] = __('Slug', 'themify');
