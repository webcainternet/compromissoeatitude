...
for($j=0;$j<sizeof($permalist);$j++) {
				...
				echo "<div id=\"fragment-$j\" class=\"fragment-slide\" style=\"\" style=\"\top: -15px;\"><a href=\"$permalist[$j]\" >$thumb[$j]</a>";
				
				if (($options['slider-display-title'])==true || ($options['slider-display-desc'])==true) {
					if($options['slider-parallax-enable']==false) {
						echo "<div class=\"info\">";
						if (($options['slider-display-title'])==true) {
							echo "<h2><a href=\"$permalist[$j]\" target=\"_parent\">$titlelist[$j]</a></h2>";
						}
						if(($options['slider-display-desc'])==true) {
								echo "<p><a href=\"$permalist[$j]\" target=\"_parent\">$contentlist[$j]</a></p>";
						}
						echo "</div>";
					}
				}
